...
/** @lends Biojs.InteractionsTable# */
{
    ...
    setDataSource: function ( dataSet ) {
        ...
        Biojs.console.log("Setting datasource: "+ dataSet);
    	if ( dataSet instanceof Array ) {
            Biojs.console.log("Setting datasource from array...");
    		// Local data
        	...
        	if ( dataSet.hasOwnProperty("totalRecords") && dataSet.hasOwnProperty("url") ) {
                Biojs.console.log("Setting datasource from remote data with record data...");
        		// Already have the number of records for pagination 
        		// then, invoke parent's setDataSource
        		this.base( dataSet );
        		
        	} else {
                Biojs.console.log("Setting datasource from remote data...");
        		// Set the new query
        		this.opt.dataSet = {};
                this.opt.dataSet.dataType = dataSet.dataType;
	...
	_useVersion: function (version) {
		...
		switch (length) {
			...
			case Biojs.InteractionsTable.MITAB_VERSION_2_5:
				...
				columnsToHide = columnsToHide.concat([2,3,7,8,12,13]);
